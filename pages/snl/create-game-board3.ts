...
export const createGameBoards3 = ({
    canvas,
    snlSimInput,
    simContext,
}: {
    ...
    const lightBoard = createGameBoard3({
        canvas,
    ...
    const darkBoard = createGameBoard3({
        canvas,
    ...
    darkBoard.group.set({
        ...
        top:
            ...
            lightBoard.group.getBoundingRect().height +
            30,
    });
...
const createGameBoard3 = ({
    ...
    const numRows = rowData ? rowData.length : 10;
    // const numCols = 10;
    const boxWidth = 30;
    const boxHeight = 30;
    ...
    for (let row = 0; row < numRows; row++) {
        const numCols = rowData ? rowData[row].length : 10;
        for (let col = 0; col < numCols; col++) {
            ...
            let colShift = 0;
            if (rowData) {
                value = rowData[row][col];
                //colShift = row;
            }
            const position: T_point2d = {
                x: (col + colShift) * boxWidth,
                y: -row * boxHeight,
            };
